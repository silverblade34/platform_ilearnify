                ...
                <v-card>
                    <div class="grid grid-cols-4 justify-space-between">
                        <div class="pt-3 col-span-3">
                            ...
                            <v-card-text>
                                <p class="text-xs">
                                    <span class="font-bold text-gray-500">Curso:</span> {{ exam.course_title }} <br>
                                    <span class="font-bold text-gray-500">Unidad:</span> {{ exam.unit_name }}
                                </p>
                            </v-card-text>
                            <v-card-actions>
                                <v-btn class="ms-2" size="small" variant="outlined" @click="goSolveExam(exam.exam_id)"
                                    color="cyan-darken-1">
                                    Realizar examen
                                </v-btn>
                            </v-card-actions>
                        ...
                        <div class="flex justify-center items-center h-full">
                            <v-img src="https://cdn-icons-png.flaticon.com/512/4696/4696755.png"
                                class="col-span-1"></v-img>
                        </div>
                    </div>
                </v-card>
            </template>
...
<script>
import { findAllExamApi } from '@/api/student/ExamService';
...
export default ({
    setup() {
        ...
        const readyData = async () => {
            findAllExamApi(store.state.token)
                .then(response => {
