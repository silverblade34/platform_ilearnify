                    ...
                    <div class="py-2">
                        ...
                        <v-radio-group v-model="answers[question.question_id]">
                            <v-radio :label="answer.answer_text" density="compact" v-for="answer in question.answers"
                                :key="answer.answer_id" :value="answer.answer_id" class="py-2"
                                color="cyan-darken-1"></v-radio>
                        </v-radio-group>
                    </div>
                </template>
                <div class="py-5">
...
export default ({
    setup() {
        ...
        const readyData = async () => {
            findOneExamApi(store.state.token, routeParams.value.id)
                .then(response => {
        ...
        const submitAnswers = () => {
            const formattedAnswers = Object.entries(answers.value).map(([question_id, answer_id]) => ({
            ...
            console.log(payload);
            submitExam(store.state.token, payload)
                .then(response => {
                    basicAlert(async () => {
                        previousView();
